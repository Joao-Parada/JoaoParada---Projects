  ...
  setTimeout(async () => {
    try {
      ...
      if (!Array.isArray(proposals) || proposals.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="4">No proposals found.</td></tr>`;
        return;
      }
      ...
      proposals.forEach((proposal) => {
        ...
        row.innerHTML = `
          ...
          <td>${proposal.generalDescription}</td>
      ...
      document.querySelector("#proposals-table tbody").innerHTML = `
        <tr><td colspan="4">Error loading proposals. Please try again later.</td></tr>
      `;
    }
  }, 0);
}
